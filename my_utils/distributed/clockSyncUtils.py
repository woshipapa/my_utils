...
class ClockSynchronizer:
    ...
    def sync(is_server: bool, peer_rank: int, group=None):
        ...
        tensor_buffer = torch.zeros(
            2, dtype=torch.float64, device=torch.cuda.current_device()
        )
        ...
        if is_server:
            ...
            print("[ClockSync] Server ready. Reference time provided.")
            return 0.0
            ...
            print("=" * 40)
            print("[ClockSync] Client Synchronization Result:")
        ...
        try:
            ...
            if is_server:
                ...
                if len(data) == 16:
                    ...
                    print(
                        f"[SocketSync] RTT: {rtt * 1000:.3f}ms, Offset: {offset:.6f}s"
                    )
                    return offset
                else:
            ...
            if self.conn:
                self.conn.close()
            if is_server and "server_sock" in locals():
                server_sock.close()
