...
from ...tracing.nvtx_utils import LabelerProtocol, NoOpLabeler
...
class CaptureController:
    def __init__(
        ...
        *,
        ...
        self._window_labeler = (
            window_labeler
            if window_labeler is not None
            else NoOpLabeler(default_domain=nvtx_domain)
        )
    ...
    def arm(self, spec: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            self._start_profile_name_set = self._to_str_set(
                self._spec.get("start_profile_names", []) or []
            )

            self._stop_profile_name_set = self._to_str_set(
                self._spec.get("stop_profile_names", []) or []
            )

            self._ranks_filter_set = self._to_int_set(
                self._spec.get("ranks_filter", None)
            )
    ...
    def _match_common(
        ...
        *,
        ...
        targets: Set[str] = (
            profile_name_set
            if profile_name_set is not None
            else set(profile_names or [])
        )

        if targets and event.profile_name not in targets:
            if debug:
                self._log("[Capture][Match] FAIL name")
        ...
        if expected_iter is not None:
            ...
            if it is None or int(it) != int(expected_iter):
                if debug:
                    self._log(
                        f"[Capture][Match] FAIL iter got={it} expected={expected_iter}"
                    )
        ...
        if expected_mb is not None:
            ...
            if mb is None or int(mb) != int(expected_mb):
                if debug:
                    self._log(
                        f"[Capture][Match] FAIL mb got={mb} expected={expected_mb}"
                    )
        ...
        if exp_role is not None:
            if event.role is None or str(event.role) != str(exp_role):
                if debug:
                    self._log(
                        f"[Capture][Match] FAIL role got={event.role} expected={exp_role}"
                    )
        ...
        active_ranks_set = (
            ranks_set
            if ranks_set is not None
            else self._to_int_set(spec.get("ranks_filter", None))
        )
        ...
        if active_ranks_set is not None:
            if event.rank is None:
                if debug:
                    self._log(
                        f"[Capture][Match] FAIL rank None expected in {sorted(active_ranks_set)}"
                    )
            ...
            if int(event.rank) not in active_ranks_set:
                if debug:
                    self._log(
                        f"[Capture][Match] FAIL rank got={event.rank} expected in {sorted(active_ranks_set)}"
                    )

                return False

        if debug:
            self._log("[Capture][Match] ? PASS")
    ...
    def _start_window(self, event: HookEvent) -> None:
        ...
        self._log(
            f"[Capture] START window={self._window_id} by={event.profile_name} meta={event.meta}"
        )
    ...
    def _stop_window(self, event: HookEvent) -> None:
        ...
        self._log(
            f"[Capture] STOP window={self._window_id} by={event.profile_name} meta={event.meta}"
        )
