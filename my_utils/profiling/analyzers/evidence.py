...
def fuse_claims(claims: Iterable[Claim]) -> Dict[str, FusedAttribute]:
    ...
    for attribute, group in grouped.items():
        ...
        agreeing = [c for c in ordered if _comparable(c.value) == _comparable(winner.value)]
        dissenting = [
            c for c in ordered
        ...
        elif winner.rank >= PROVENANCE_RANK[Provenance.NVTX] and len(distinct_sources) > 1:
...
def claims_from_kernel_name(kernel_name: str) -> List[Claim]:
    ...
    truncated = raw.endswith(("...", "…")) or (len(raw) >= 180 and "<" in raw and ">" not in raw[-40:])

    out.append(Claim("category", info.category, Provenance.KERNEL_NAME,
                     detail=f"symbol {raw[:60]!r}", advisory=truncated))
    if info.framework:
        ...
        out.append(Claim("framework", info.framework, Provenance.KERNEL_NAME,
    ...
    if gemm is not None and getattr(gemm, "tile_m", None):
        out.append(Claim("gemm_tile", (gemm.tile_m, gemm.tile_n, gemm.tile_k),
                         Provenance.KERNEL_NAME, detail="tile tokens in symbol",
                         advisory=truncated))

    nccl = parse_nccl_kernel(raw)
    if nccl is not None and nccl.collective:
        out.append(Claim("collective", nccl.collective, Provenance.KERNEL_NAME,
                         detail="NCCL symbol", advisory=truncated))
        if nccl.algorithm:
            out.append(Claim("nccl_algorithm", nccl.algorithm, Provenance.KERNEL_NAME,
                             detail=nccl.algorithm_caveat() or "NCCL symbol",
                             advisory=not nccl.algorithm_is_authoritative))
        if nccl.protocol:
            out.append(Claim("nccl_protocol", nccl.protocol, Provenance.KERNEL_NAME,
                             detail=nccl.algorithm_caveat() or "NCCL symbol",
                             advisory=not nccl.algorithm_is_authoritative))
    ...
    if low.startswith("kernel_kernel_") or (
        ...
        out.append(Claim("name_is_informative", False, Provenance.KERNEL_NAME,
                         detail=("CuTe DSL default naming: tensor args are IR values and are "
    ...
    *,
    ...
    if launch_api:
        ...
        if api.startswith("cuLaunch"):
            out.append(Claim("jit_compiled", True, Provenance.CUDA_API,
                             detail=f"{api}: kernel came from a runtime-loaded module"))
        elif api.startswith("cudaLaunch"):
            out.append(Claim("jit_compiled", False, Provenance.CUDA_API,
    ...
    if shared_mem_bytes is not None and shared_mem_bytes > 0:
        ...
        out.append(Claim("shared_mem_bytes", int(shared_mem_bytes), Provenance.LAUNCH_CONFIG))
        if shared_mem_bytes > 100 * 1024:
            out.append(Claim("uses_smem_carveout", True, Provenance.LAUNCH_CONFIG,
    ...
    if registers_per_thread:
        out.append(Claim("registers_per_thread", int(registers_per_thread),
...
def claims_from_counters(metrics: Any) -> List[Claim]:
    ...
    if tensor is not None:
        used = tensor > 1.0
        out.append(Claim("uses_tensor_cores", used, Provenance.HW_COUNTER,
                         detail=f"tensor pipe at {tensor:.1f}% of peak"))

    tma = view.get("pipe_tma_util")
    if tma is not None:
        out.append(Claim("uses_tma", tma > 0.0, Provenance.HW_COUNTER,
                         detail=f"TMA pipe at {tma:.2f}% of peak"))

    fp64 = view.get("pipe_fp64_util")
    if fp64 is not None and fp64 > 1.0:
        out.append(Claim("uses_fp64", True, Provenance.HW_COUNTER,
    ...
    if green is not None:
        out.append(Claim("green_context", True, Provenance.HW_COUNTER,
    ...
    if passes is not None and passes > 1:
        out.append(Claim("replay_passes", int(passes), Provenance.HW_COUNTER,
                         detail=f"kernel was replayed {int(passes)} times; metrics from "
                                "different passes are not simultaneous"))
    return out


def claims_from_nvtx(ranges: Sequence[Any] = (), payloads: Optional[Dict[str, Any]] = None) -> List[Claim]:
    ...
    for rng in ranges or ():
        ...
        if name.startswith("nccl"):
            out.append(Claim("collective", name[4:].lower(), Provenance.NVTX,
    ...
    for key, value in (payloads or {}).items():
        ...
        if label == "Collective name":
            out.append(Claim("collective", str(value).lower(), Provenance.NVTX,
                             detail="PyTorch/Kineto collective metadata"))
        elif label in ("Message size [bytes]", "In msg nelems"):
            out.append(Claim("message_bytes", value, Provenance.NVTX,
                             detail="NVTX payload: size of ONE message, not aggregate bytes"))
        elif label in ("Group size", "Process Group Ranks"):
            out.append(Claim("group_size", value, Provenance.NVTX, detail="NVTX payload"))
        elif label == "Reduction operation":
            out.append(Claim("reduction_op", str(value).lower(), Provenance.NVTX,
                             detail="NVTX payload"))
    return out
...
def attribute_kernel(
    ...
    **launch: Any,
    ...
    if (
        category is not None and str(category.value) == "matmul"
        and tensor is not None and tensor.value is False
    ):
        warnings.append(
