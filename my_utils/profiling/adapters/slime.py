...
class SlimeAdapter(FrameworkAdapter):
    ...
    def detect(self, context: Mapping[str, Any]) -> bool:
        ...
        if not command:
            return False
        return "thudm/slime" in command or "/slime/" in command or " slime/" in command
    ...
    def build_runtime_tags(self, context: Mapping[str, Any]) -> Dict[str, str]:
        return {"framework": self.name, "adapter": "SlimeAdapter"}
