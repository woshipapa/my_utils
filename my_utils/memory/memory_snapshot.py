    ...
    def __init__(self, save_dir="memory_snapshots"):
        ...
        if self.is_enabled:
            if not (torch.cuda.is_available()):
                ...
                self.logger.info(
                    f"MemorySnapshotter is ENABLED. 快照将保存到: {self.save_dir}"
                )
        else:
    ...
    def start(self, name: str, max_entries=100000):
        ...
        try:
            if not self._active_names:
                ...
                self.logger.info(
                    f"[MemSnapshot] START-ROOT: '{name}' (已启动并重置历史)"
                )
            else:
                # 已经是子调用, 确保记录器仍在运行
                # torch.cuda.memory._record_memory_history(enabled=True)
            ...
            self.logger.error(
                f"[MemSnapshot] FAILED to start recording for '{name}': {e}"
            )
    ...
    def stop(self, name: str):
        ...
        if not self._active_names or self._active_names[-1] != name:
            ...
            if name not in self._active_names:
                self.logger.error(
                    f"[MemSnapshot] FAILED to stop '{name}'. 不在活动堆栈中。"
                )
                return
            # 弹出所有子节点, 直到找到它
        ...
        try:
            ...
            if not self._active_names:
                self.logger.info(
                    "[MemSnapshot] STOP-ROOT: 所有快照已停止。正在重置内存历史。"
                )
                torch.cuda.memory._record_memory_history(enabled=None)  # 重置

        except Exception as e:
            self.logger.error(
                f"[MemSnapshot] FAILED to dump snapshot for '{name}': {e}"
            )
