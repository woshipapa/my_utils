...
class GPU_Performance_Tracker:
    def __init__(
        ...
        track_hardware_metrics: bool = True,
    ):
        """
        初始化 GPU 性能监控类
        :param prefix: 文件名前缀
        ...
        if self.track_hardware_metrics:
            try:
                ...
                print(
                    f"[Rank {self.rank}] Warning: Failed to initialize PyNVML. Hardware metrics will be disabled. Error: {e}"
                )
        ...
        while self._is_running:
            ...
            if self.track_hardware_metrics:
                try:
                    ...
                    print(
                        f"[Rank {self.rank}] Warning: NVMLError while sampling hardware metrics: {e}"
                    )
                    # 采样失败时填充默认值，保持数据对齐
    ...
    def start_monitoring(self):
        ...
        if not self._is_running:
            ...
            self._monitor_thread = threading.Thread(
                target=self._monitor_metrics, daemon=True
            )
    ...
    def stop_monitoring(self):
        ...
        print(
            f"[Rank {self.rank}]: Peak Memory -> Allocated: {max_alloc_gb:.2f} GB, Reserved: {max_reserv_gb:.2f} GB"
        )
    ...
    def plot_usage_graphs(self):
        ...
        ax1.plot(
            self.timestamps,
            self.allocated_memory_gb,
            label="PyTorch Allocated Memory (GB)",
            color="blue",
        )
        ax1.plot(
            self.timestamps,
            self.reserved_memory_gb,
            label="PyTorch Reserved Memory (GB)",
            color="cyan",
            linestyle="--",
        )
