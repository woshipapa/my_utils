...
class ProfilerWrapper:
    ...
    def record_cuda_mm(self):
        ...
        print(
            f"counter : {self.counter} : allocated_mm = {allocated}GB, reserved_mm = {reserved}GB"
        )
    ...
    def record_cuda_average_time(self):
        ...
        total_time = (
            spatial_total_time + temporal_total_time + transformer_total_time
        ) / 1000
    ...
    def plot_memory_line(self):
        ...
        with open(f"{self.prefix}_memory_usage.txt", "a") as f:
            f.write(
                f"Allocated Memory (GB)  (device = {self.device})  average_slope = {average_slope}\n"
            )
    ...
    def plot_cuda_times(self):
        ...
        plt.plot(
            self.run_indices, self.cuda_times, marker="o", label="Self CUDA Time (ms)"
        )
        ...
        plt.figtext(
            0.1, 0.1, f"Average Time: {average_time:.4f} ms", ha="left", fontsize=10
        )
        plt.figtext(
            0.1,
            0.07,
            f"FA Average Time: {fa_average_time:.4f} ms",
            ha="left",
            fontsize=10,
        )
        plt.figtext(
            0.1, 0.04, f"FA / Average Time Ratio: {ratio:.4f}", ha="left", fontsize=10
        )
    ...
    def plot_bar_chart(self, average_time_ms, fa_average_time_ms):
        ...
        for i, value in enumerate(values):
            plt.text(
                i, value + 0.5, f"{value:.2f} ms", ha="center", va="bottom", fontsize=10
            )
    ...
    def plot_top5_cuda_time(self, save_path=None):
        ...
        sorted_averages = sorted(
            key_averages, key=lambda avg: avg.self_cuda_time_total, reverse=True
        )
        ...
        times = [avg.self_cuda_time_total for avg in top5] + [
            sum(avg.self_cuda_time_total for avg in other)
        ]
