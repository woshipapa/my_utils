...
class FlexibleProfiler:
    ...
    def _trace_handler(self, p):
        ...
        if self.task_type == "VAE":
            self.logger.info(
                f"任务类型为 VAE，跳过生成 JSON Trace 文件 (Rank {self.rank})."
            )
        else:
            # 只有当任务类型不是 VAE 时，才导出 JSON trace
            trace_file = os.path.join(
                self.output_dir, f"torch_prof_rank{self.rank}.json"
            )
            p.export_chrome_trace(trace_file)
            self.logger.info(
                f"Profiler trace for rank {self.rank} saved to {trace_file}"
            )

        # --- Summary 的生成逻辑保持不变，不受影响 ---
        summary_file = os.path.join(
            self.output_dir, f"prof_summary_rank{self.rank}.txt"
        )
        with open(summary_file, "w") as f:
            f.write(
                p.key_averages().table(sort_by="self_cuda_time_total", row_limit=100)
            )
        self.logger.info(
            f"Profiler summary for rank {self.rank} saved to {summary_file}"
        )
    ...
    def _analyze_and_persist_ops(self):
        ...
        """
        self.logger.info(
            f"Performing detailed analysis (grouped by unique name) for specified ops on Rank {self.rank}..."
        )
        ...
        for full_name, group_data in events_grouped_by_name.items():
            ...
            if non_zero_values_us:
                ...
                self.logger.info(
                    f"  - Analyzed '{full_name}': Found {count} instances, Avg: {average_ms:.3f} ms."
                )
            else:
                self.logger.warning(
                    f"  - For '{full_name}', all recorded instances had a zero value for metric '{metric_to_use}'."
                )
        ...
        if final_analysis:
            output_file = os.path.join(
                self.output_dir, f"detailed_metrics_rank_{self.rank}.json"
            )
            with open(output_file, "w") as f:
                json.dump(
                    final_analysis, f, indent=4, sort_keys=True
                )  # 按key排序更美观
